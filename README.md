...
Das Plug-in besitzt in Version `0.4.14.0` ein kompaktes schwebendes, vierstufig gerahmtes Zuweisungsfenster. Es
...
Version 0.4.14.0 verwendet innerhalb der bereits laufenden `LrTask` wieder direkt `withWriteAccessDo` und führt die
einzelnen SDK-Aufrufe im Lightroom-Schreibcallback aus. Zusätzliche Fehlerkapsel, Timeout und die irreführende
Übersetzung als Katalogbelegung wurden entfernt. Callback-Abschluss und gespeicherte `masterTaxonId` werden vor der
Erfolgsmeldung ausdrücklich geprüft.
