...
selbst unter `#species-sound`, damit Spektrogramm, Canvas-Fallback und Play-Bedienung nicht vom aktuellen
...
| `.sound-player`, `.sound-spectrogram-image`, `.sound-wave-canvas`, `.sound-cursor`, `.sound-scrubber` | Spektrogramm-Anzeige, Canvas-Fallback und Scrubbing ohne externe Waveform-Bibliothek. | Niedrig. | Werden durch `species-sound.js` unter `#species-sound` gekapselt injiziert. |
...
- Soundbox, Spektrogramm und Canvas-Fallback passen in die Breite.
...
- Soundbox, Spektrogramm und Canvas-Fallback bleiben im Viewport.
