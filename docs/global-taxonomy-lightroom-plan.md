...
10.2 bis 10.4 sind bis Plug-in-Version 0.4.13.0 umgesetzt und automatisiert geprüft. Einzel- und Mehrfachzuweisung,
Fensteraufbau, Favoritenersetzung und Taxonomierücknahme wurden im separaten Lightroom-Testkatalog praktisch
geprüft; der 0.4.13.0-Reparaturstand benötigt noch den praktischen Folgetest. Phase 10 bleibt bis zum
...
- **Bis Plug-in-Version 0.4.13.0 am 2026-08-29 umgesetzt und automatisiert geprüft; grundlegende Einzel- und
  Mehrfachzuweisung praktisch bestätigt, der 0.4.13.0-Reparaturstand noch nicht praktisch abgenommen.**
...
- Vor dem Lightroom-Schreibzugriff entsteht aus den sichtbaren `(FN)`-Namen eine eindeutige Liste. Gleich benannte
  ...
  Operation verwendet `LrTasks.pcall`; die einzelnen SDK-Aufrufe laufen direkt im Lightroom-Schreibcallback.
  Version 0.4.13.0 entfernt den optionalen Timeout aus 0.4.11.0, korrigiert den in 0.4.12.0 syntaktisch falsch
  geschlossenen Callback und prüft vor einer Erfolgsmeldung Callback-Abschluss sowie gespeicherte `masterTaxonId`.
...
- Der wiederholbar einrichtbare Sammlungssatz `FN Wildlife & Travel` enthält `Art-Favoriten`, `Taxonomie fehlt`
  und `Taxonomie zugewiesen`. Die Regeln verwenden ausschließlich `referenceImage` und `masterTaxonId` aus den
  Plug-in-Metadaten. Eine gültige Taxonomie wird am reservierten Master-ID-Präfix `mtx_` erkannt;
  `Taxonomie fehlt` ist über `exclude` die Gegenmenge. Die praktisch umgekehrt ausgewerteten Leerheitsoperationen
  werden vermieden. Normale Fotometadaten und
...
im Testkatalog bestätigt. Phase 10.3/10.4 ergänzen bis Version 0.4.13.0 die nutzergeführte Rücknahme, dynamische
